...
// ------------------------------------------------
// Single source of truth for the Gemini Prompt Caching reference
// implementation used by PromptPilot AI. The Python code string below
// is what the "Copy Production Python Code" button copies, and
// `caching_pipeline.py` at the repo root is generated from the same
// mental model (kept in sync manually).
//
// Why 3 layers?
//   Layer 1 (Static)        - System persona + scoring rubric. Never changes.
//                             Fully cacheable. Cached for hours/days.
//   Layer 2 (Developer Rules) - Domain frameworks + mode instructions.
//                             Change weekly/monthly. Cacheable.
//   Layer 3 (Dynamic)       - User prompt + attachments.
//                             Changes every call. NOT cached.
//
// On Gemini, identical prefixes + a short TTL = up to ~75% cost reduction
// on cached input tokens and significantly lower latency.

export const PIPELINE_MODEL = 'gemini-1.5-flash-002';

// Layer 1 - Static System Instructions (Fully Cacheable)
// These tokens are identical across every call, so the entire
// prefix becomes a stable cache key on Gemini.
export const LAYER_1_SYSTEM_INSTRUCTIONS = `You are PromptPilot, an expert prompt-engineering copilot that rewrites weak or vague prompts into precise, high-signal instructions for large language models.
...
// Layer 2 - Reference Context / Developer Rules (Cacheable)
// Changes infrequently per project. Good candidate for a longer-TTL cache
// that you invalidate only when the rules themselves change.
export const LAYER_2_DEVELOPER_RULES = `Domain frameworks:
...
export const PIPELINE_LAYERS = [
  {
    id: 'layer-1',
    badge: 'L1',
    title: 'Static System Instructions',
    subtitle: 'Fully Cacheable',
    description:
      'Persona, output contract, and scoring rubric. Identical across every call \u2014 Gemini caches the prefix verbatim.',
    cacheable: true,
    accent: '#a78bfa',
    preview: LAYER_1_SYSTEM_INSTRUCTIONS,
  },
  {
    id: 'layer-2',
    badge: 'L2',
    title: 'Reference Context / Developer Rules',
    subtitle: 'Cacheable',
    description:
      'Domain frameworks and mode overlays. Invalidate the cache only when the rules change.',
    cacheable: true,
    accent: '#34d399',
    preview: LAYER_2_DEVELOPER_RULES,
  },
  {
    id: 'layer-3',
    badge: 'L3',
    title: 'Dynamic User Input',
    subtitle: 'Non-Cacheable',
    description:
      "The user's raw prompt plus any attached files. Always sent fresh \u2014 this is what makes every response unique.",
    cacheable: false,
    accent: '#f59e0b',
    preview: null,
  },
...
export const PIPELINE_BENEFITS = [
  {
    label: 'Cost',
    value: '~75% less',
    detail: 'Cached input tokens are billed at a discount on Gemini.',
  },
  {
    label: 'Latency',
    value: 'Lower TTFT',
    detail: 'Cache hits skip prefix re-processing.',
  },
  {
    label: 'Maintenance',
    value: 'Decoupled',
    detail: 'Static rules live in one versioned module.',
  },
];

// Production-ready Gemini Prompt Caching reference implementation.
// Kept here as a single source of truth so the "Copy" button and the
// standalone `caching_pipeline.py` reference stay aligned.
export const PRODUCTION_PYTHON_CODE = `"""
...
# Layer 1: Static System Instructions (Fully Cacheable)
# ---------------------------------------------------------------------------
LAYER_1_SYSTEM_INSTRUCTIONS = """\${L1_PLACEHOLDER}"""


# ---------------------------------------------------------------------------
# Layer 2: Developer / Reference Rules (Cacheable)
# ---------------------------------------------------------------------------
LAYER_2_DEVELOPER_RULES = """\${L2_PLACEHOLDER}"""
...
class ThreeLayerPromptPipeline:
    ...
    def get_or_create_cache(self, rules: str) -> CachedPipeline:
        ...
        if (
            ...
            and self._cache.age_seconds < int(self.ttl.rstrip("s"))
        ):
            return self._cache
